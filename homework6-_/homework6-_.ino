...
void setup_runtime(){
  ...
  http.begin("http://54.208.105.157:8086/write?db=sensorDB");
  if(MDNS.begin("JYJ")){
...
void loop(){
  ...
  int httpCode1 = http.POST("ne,host=server01,region=us-west value=" + String(temperature));
  int httpCode2 = http.POST("ne,host=server02,region=us-west value=" + String(humidity));
  int httpCode3 = http.POST("ne,host=server03,region=us-west value=" + String(Light));
